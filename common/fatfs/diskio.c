...
DSTATUS MMC_disk_initialize() { return STA_NODISK; }

DSTATUS USB_disk_initialize() { return STA_NODISK; }

DSTATUS RAM_disk_read(BYTE *buf, LBA_t sector, UINT count);

DSTATUS MMC_disk_read(BYTE *buf, LBA_t sector, UINT count) {
  return STA_NODISK;
}

DSTATUS USB_disk_read(BYTE *buf, LBA_t sector, UINT count) {
  return STA_NODISK;
}

DSTATUS RAM_disk_write(const BYTE *buf, LBA_t sector, UINT count);

DSTATUS MMC_disk_write(const BYTE *buf, LBA_t sector, UINT count) {
  return STA_NODISK;
}

DSTATUS USB_disk_write(const BYTE *byte, LBA_t sector, UINT count) {
  return STA_NODISK;
}
...
DRESULT disk_write(BYTE pdrv, /* Physical drive nmuber to identify the drive */
  ...
  case DEV_MMC:
    ...
    // translate the result coude here
