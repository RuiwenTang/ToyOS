...
static void prn_str(char *print_buf, size_t *print_buf_i, const char *string) {
  ...
  for (i = 0; string[i]; i++) {
    if (*print_buf_i == (PRINT_BUF_MAX - 1)) break;
...
static void prn_nstr(char *print_buf, size_t *print_buf_i, const char *string,
                     size_t len) {
  size_t i;
...
void vprint(const char *fmt, va_list args) {
  ...
  for (;;) {
    while (*fmt && *fmt != '%') prn_char(print_buf, &print_buf_i, *fmt++);
    ...
    switch (*fmt++) {
      ...
      case '#': {
        bool printed = false;
        char *str = (char *)va_arg(args, const char *);
        for (int i = (int)_strlen(str) - 1; i >= 0; i--) {
          if (str[i] != '#') {
            continue;
          }
